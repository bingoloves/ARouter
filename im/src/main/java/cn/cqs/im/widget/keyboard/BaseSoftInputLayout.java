    ...
    private void detectKeyBoardState() {
        ...
        if (coverHeight > mNavigationBarHeight) {
            ...
            if (mKeyboardOpenListener != null){
                mKeyboardOpenListener.onKeyboard(true);
            }
        } else {
            if ((deltaCoverHeight == mNavigationBarHeight || deltaCoverHeight == -mNavigationBarHeight) && !mIsKeyboardShow) {
                // 华为显示/隐藏navigationBar
            ...
            if (mKeyboardOpenListener != null){
                mKeyboardOpenListener.onKeyboard(false);
            }
        }
    }
...
//                showWhat = 0;
//                hideSoftInput();
                //将默认的再次点击键盘的按钮事件替换成显示EmotionView
                ViewHolder viewHolder = viewMapping.get(v);
                if (viewHolder != null) {
                    showWhat = viewHolder.getSHOW_TYPE();
            ...
            if (viewHolder != null) {
                ...
                if (showWhat == SHOW_OTHER) {//这里再次点击other 切换成键盘模式
                    showWhat = SHOW_KEYBOARD;
                    hideView(showView);
                    showSoftInput();
                    showView(container);
...
//                }
    ...
    public void setKeyboardOpenListener(KeyboardOpenListener mKeyboardOpenListener) {
